...
const findSolution = (draws, boards) => {
    ...
    let lastWinningBoardScore = null;

    for (let draw of draws) {
        drawnSet.add(draw);
        const winningBoard = remainingBoards.find(board => checkBoard(board, drawnSet));
        if (winningBoard) {
            lastWinningBoardScore = getBoardScore(winningBoard, drawnSet) * draw;
        }
        remainingBoards = remainingBoards.filter(board => !checkBoard(board, drawnSet));
    }
